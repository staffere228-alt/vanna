...
do
    ...
    local LocalPlayerFix = Players.LocalPlayer
    while not LocalPlayerFix do
        wait()
        LocalPlayerFix = Players.LocalPlayer
    end

    local reconnectDelay = 2
    pcall(function()
        if SETTINGS and SETTINGS.ReconnectDelay then
    ...
    local function isDisconnectText(text)
        ...
        for _, phrase in ipairs(RECONNECT_PHRASES) do
            if lower:find(phrase, 1, true) or text:find(phrase, 1, true) then
    ...
    local function forceReconnectFix(reason)
        ...
        -- Страховка от вечного reconnectFixActive = true
        spawn(function()
            wait(25)
            reconnectFixActive = false
        end)
        ...
        local function scanObject(obj)
            pcall(function()
                ...
                local function checkText()
                    ...
                    if hit then
                        forceReconnectFix("Prompt: " .. tostring(why))
                    end
                end
    ...
    pcall(function()
        Players.PlayerRemoving:Connect(function(player)
            if player == Players.LocalPlayer then
                forceReconnectFix("PlayerRemoving")
            end
        end)
    end)
    ...
    pcall(function()
        ...
        if lp then
            lp.OnTeleport:Connect(function(state)
                if state == Enum.TeleportState.Failed or state == Enum.TeleportState.Started then
                    forceReconnectFix("OnTeleport: " .. tostring(state))
                end
            end)
        end
    end)
    ...
    RunService.Heartbeat:Connect(function()
        if not Players.LocalPlayer or not Players.LocalPlayer.Parent then
            consecutiveFailuresFix = consecutiveFailuresFix + 1

            if consecutiveFailuresFix >= 5 then
                forceReconnectFix("Heartbeat: LocalPlayer missing")
            end
        else
            consecutiveFailuresFix = 0
        end
    end)
